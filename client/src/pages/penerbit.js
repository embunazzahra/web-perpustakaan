...
const Penerbit = () => {
  ...
  const getDataPenerbit = async (id_penerbit) => {
    const body = id_penerbit;
    try {
      const response = await fetch("http://localhost:4000/penerbit", {
        method: "POST",
  ...
  useEffect(() => {
    getDataPenerbit(location.state);
                ...
                <tbody className="bg-white divide-y divide-gray-300">
                  {dataPenerbit.map((penerbit) => (
                    <tr className="whitespace-nowrap">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 font-poppins">
                          {penerbit.email_penerbit}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 font-poppins">
                          {penerbit.nomor_tel_penerbit}
                        </div>
                      </td>
                    </tr>
                  ))}
